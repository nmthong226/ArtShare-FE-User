...
import { CommentUI } from "./PostComment.types";
  ...
  const toggleReplies = async () => {
    if (!showReplies && comment.replies === undefined) {
      try {
        ...
        onRepliesFetched(comment.id, data);
      } catch (err) {
        console.error(err);
  ...
  ({ comments: initial, postId }, _ref) => {
    ...
    const handleAdd = async () => {
      ...
      const optimistic: CommentUI = {
        ...
        content,
        created_at: now,
        updated_at: now, // Ensure this is a Date object
        replies: [],
        likes: 0,
        likedByUser: false,
      };
      ...
      setIsPosting(true);
      try {
        const { data } = await api.post<CommentUI>("/comments/create", {
          content,
          target_id: postId,
          target_type: "POST",
        ...
        <div className="flex flex-col max-h-[480px] overflow-y-auto divide-y divide-neutral-100">
          {comments.map((c) => (
            <CommentRow
              key={c.id}
              comment={c}
              onLike={(id) => setComments((p) => toggleLikeRecursive(p, id))}
              onReply={(id, username) => {
                setReplyParentId(id);
