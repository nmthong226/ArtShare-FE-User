...
export const SavePostDialog = (props: SavePostDialogProps) => {
    ...
    return (
        <Dialog open={open} scroll="paper" aria-labelledby="scroll-dialog-title" aria-describedby="scroll-dialog-description">
            ...
            <DialogContent dividers={true} className="flex flex-col gap-4 px-0 w-[600px]">
                {isCreatingNewCollection ? (
                    <div className="flex flex-col gap-1 px-4">
                        <Input placeholder="Collection name" className="shadow-inner px-2 py-1 border-1 rounded w-full" disableUnderline />
                        <FormControlLabel
                        ...
                        <div className="flex justify-between">
                            <Button variant="contained" className="shadow-none mt-4 font-normal normal-case" onClick={handleSaveCollection}>
                                Create
                            </Button>
                            <Button variant="outlined" className="shadow-none mt-4 font-normal normal-case" onClick={handleSaveCollection}>
                                Cancel
                            </Button>
                        </div>
                    </div>
                    ...
                    <>
                        <div className="px-4">
                            <Button variant="outlined" className="flex items-center gap-2 p-2" onClick={handleCreateNewCollection}>
                                <PlusCircleIcon />
                                <span className="font-normal text-md normal-case">Create new collection</span>
                            </Button>
                        </div>
                        <div className="px-4">
                            ...
                            {filteredCollections.map((collection, index) => (
                                <div className="flex items-center gap-2 p-2 px-4 border-mountain-400 border-b" key={index}>
                                    ...
                                    {collection.postIds.includes(postId) ? (
                                        <Button variant="outlined" className="flex items-center gap-1 shadow-none font-normal normal-case" disableRipple>
                                            <CheckIcon size={16} />
                                            <span>Added</span>
                                        </Button>
                                    ) : (
                                        <Button variant="contained" className="shadow-none font-normal normal-case">
                                            Add to collection
                                        </Button>
                                    )}
                                </div>
