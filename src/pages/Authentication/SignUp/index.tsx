    ...
    return (
        <div className="flex-1 space-y-4 px-10 md:px-0 lg:px-20 py-8">
            <div className="flex flex-col space-x-3">
                <h1 className="font-bold text-mountain-800 dark:text-mountain-50 text-2xl xl:text-3xl leading-6">Join us!</h1>
                <p className="mt-2 font-bold text-mountain-600 dark:text-mountain-300 text-xl lg:text-2xl xl:text-3xl text-nowrap">Create an ArtShare account</p>
                <p className="mt-4 text-mountain-500 dark:text-mountain-300 text-xs xl:text-sm">Join a vibrant community where you can create, share, and celebrate art.</p>
            </div>
            ...
            <form className="space-y-4">
                <div>
                    ...
                    <Input type="text" placeholder="Enter your username or email" className="dark:bg-mountain-900 shadow-sm mt-1 p-3 border border-mountain-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full h-10" />
                </div>
                <div>
                    <label htmlFor="password" className="block font-medium text-mountain-600 dark:text-mountain-50 text-sm">Password</label>
                    <Input type="password" placeholder="Enter your password" className="dark:bg-mountain-900 shadow-sm mt-1 p-3 border border-mountain-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full h-10" />
                </div>
                <div className="flex justify-between items-center mt-4">
                    <span className="text-mountain-500 text-xs xl:text-sm">Your password must be at least 8 characters, numbers and symbols.</span>
                </div>
                <Button type="submit" className="bg-mountain-800 hover:bg-mountain-700 dark:bg-gradient-to-r dark:from-blue-800 dark:via-purple-700 dark:to-pink-900 hover:brightness-110 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full h-10 font-bold text-white dark:text-mountain-50 hover:cursor-pointer">Sign Up with Email</Button>
            </form>
            <div className="mt-6 text-left">
                <p className="text-mountain-600 text-xs xl:text-sm">Already have an account?
                    <Link to="/login" className="ml-2 text-indigo-600 dark:text-indigo-300">Login</Link>
                </p>
            </div>
            <div className="mt-4 text-[10px] text-mountain-500 xl:text-xs lg:text-left text-center">
