...
const Search = () => {
  ...
  return (
    <div className="flex flex-col w-full h-[calc(100vh-4rem)]">
      <div className="relative flex flex-col justify-end items-center space-x-4 space-y-4 bg-white pt-10 border-mountain-100 border-b-1 w-full h-fit">
        ...
        <div className="flex justify-center items-center space-x-2 mb-4">
          ...
          {/* <Button
            className="z-50 p-3 rounded-lg min-w-auto aspect-[1/1] translate spread-btn"
            variant="contained"
            disableElevation
        ...
        <div className="relative flex justify-center items-center border-mountain-100 border-t-1 w-full">
          <div className="top-1/2 left-4 absolute flex items-center space-x-4 -translate-y-1/2 transform">
            ...
            <div
              ...
              onClick={() => {
                ...
                setShowChannelDropdown(false); // Reset dropdown when hiding filter
              }}
            >
              <BsFilter size={16} />
              <p>Filter</p>
            </div>
            {/* Channels Filter Button (only shows if Filter is active) */}
            {showFilters && (
          ...
          <div className="flex">
            <div onClick={() => setTab('posts')} className={`${tab === 'posts' ? 'border-indigo-400' : 'text-mountain-600 hover:bg-mountain-50 border-white hover:border-mountain-50'} hover:cursor-pointer flex justify-center items-center  p-2 border-b-4 w-32 text-lg`}>
              Posts
            </div>
            <div onClick={() => setTab('users')} className={`${tab === 'users' ? 'border-indigo-400' : 'text-mountain-600 hover:bg-mountain-50 border-white hover:border-mountain-50'} hover:cursor-pointer flex justify-center items-center  p-2 border-b-4 w-32 text-lg`}>
              Users
            </div>
          </div>
